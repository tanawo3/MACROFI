...
export interface GenTx {
  hash: string;
  type: 'deploy' | 'adjust_rates' | 'apply_loan' | 'evaluate_loan' | 'repay_loan' | 'stake' | 'unstake';
...
export const useGenLayer = () => {
  ...
  const [contractAddress, setContractAddress] = useState<string>(
    localStorage.getItem('MACROFI_CONTRACT_ADDRESS_V2') || GLOBAL_CONTRACT_ADDRESS
  );
  ...
  const [protocolState, setProtocolState] = useState<MacroFiState>({
      current_base_rate: 0,
      last_update_rationale: "",
      update_counter: 0,
      logs: [],
      treasury: null
  });
  ...
  useEffect(() => {
    ...
    setContractAddress(localStorage.getItem('MACROFI_CONTRACT_ADDRESS_V2') || GLOBAL_CONTRACT_ADDRESS);
    setError(null);
  ...
  const deployContract = useCallback(async () => {
    ...
    try {
      ...
      let msg = e?.message || "Failed to deploy contract. Ensure your wallet is connected, set to correct chain ID, and has testnet GEN tokens.";
      msg = msg.replace(/^\[.*?\]\s*/, '');
      setError(msg);
    } finally {
    ...
    if (result) {
      try {
        ...
        setProtocolState(prev => ({
            ...prev,
            current_base_rate: parsed.current_base_rate || 0,
            last_update_rationale: parsed.last_update_rationale || "",
  ...
  const adjustRates = async () => {
      ...
      try {
          ...
          let msg = e?.message || "Failed to execute transaction.";
          msg = msg.replace(/^\[.*?\]\s*/, '');
          setError(msg);
      } finally {
  ...
  const applyForLoan = async (poolId: string, pitch: string, loanType: string, requestedAmount: number, durationDays: number, monthlyIncomeUsd: number, githubContributions: number, daoVotes: number, walletAgeDays: number, collateralAmount: number) => {
      ...
      try {
          ...
          const hash = await (client as any).writeContract({
              ...
              args: [poolId, pitch, loanType, requestedAmount, durationDays, monthlyIncomeUsd, githubContributions, daoVotes, walletAgeDays],
  ...
  const evaluateLoan = async (appId: string) => {
      if (!contractAddress) return;
      setError(null);
      try {
          const provider = window.ethereum || (window as any).okxwallet || (window as any).rabby;
  ...
  const submitIdentityVerification = async (docType: string, docHash: string, selfieHash: string, poaHash: string) => {
      if (!contractAddress) return;
      setError(null);
      try {
          const provider = window.ethereum || (window as any).okxwallet || (window as any).rabby;
          ...
          const hash = await (client as any).writeContract({
              ...
              args: [docType, docHash, selfieHash, poaHash]
          });

    ...
    <div className="flex flex-col gap-8 w-full mt-12 pt-12 border-t-2 border-[var(--border-color)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="brutalist-panel p-8">
          ...
          <div className="flex flex-col gap-4">
            <textarea placeholder="Describe your loan purpose..." rows={4} className="bg-transparent border border-zinc-700 p-3 outline-none focus:border-[var(--text-lime)] resize-none" value={pitch} onChange={e => setPitch(e.target.value)} />
            ...
            <Magnetic>
              <button onClick={() => applyForLoan('GLOBAL', pitch, parseInt(collat))} className="btn-primary w-full py-3 mt-2">SUBMIT APPLICATION</button>
            </Magnetic>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        {loans.map((loan, idx) => (
          <div key={idx} className="brutalist-panel p-6 border-l-4 border-l-[var(--text-lime)] flex flex-col gap-4">
            ...
            {loan.ai_notes && (
              <div className="mt-4 p-4 bg-black/50 border border-zinc-800 text-zinc-300 italic text-sm border-l-4 border-l-orange-500">
                <span className="block font-bold mb-1 text-orange-500 not-italic uppercase text-xs">AI Underwriter Notes:</span>
                {loan.ai_notes}
              </div>
            )}
